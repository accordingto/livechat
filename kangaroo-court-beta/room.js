// Independent Beta room: do not remove the storage or Firebase prefixes.
...
const ROOM = (() => {
  const STORAGE_PREFIX = 'kangaroo-beta-room-session-';
  const LAST_SESSION_KEY = 'kangaroo-beta-room-last-session';
  ...
  function attach() {
    ...
    if (!on()) return;
    for (let i = 0; i < count; i++) {
      const ref = db.ref(`rooms/kangaroo-beta-${sessionCode}/players/${tokens[i]}`);
      const num = i + 1;
      const handler = snap => {
        data[num] = snap.val();
  ...
  function publish(build) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      db.ref(`rooms/kangaroo-beta-${sessionCode}/players/${tokens[i]}`).set(payload);
      data[i + 1] = null;
    }
    // non-secret name list, so a card can show who else is in the room
    db.ref(`rooms/kangaroo-beta-${sessionCode}/roster`).set(
  ...
  function update(build) {
    ...
    for (let i = 0; i < count; i++) {
      const payload = typeof build === 'function' ? build(i) : Object.assign({}, build);
      if (!payload) continue;
      db.ref(`rooms/kangaroo-beta-${sessionCode}/players/${tokens[i]}`).update(payload);
    }
  }
  ...
  function clearAnswer(i) {
    ...
    db.ref(`rooms/kangaroo-beta-${sessionCode}/players/${tokens[i]}/vote`).set(null);
