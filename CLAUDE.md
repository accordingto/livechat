...
| `dont-say-it.html` | 🙊 Don't Say It!（Taboo 玩法。選 2–6 人，按「🎬 Deal a Word」抽一張卡並指定這輪的 clue giver（每輪自動輪替到下一位）：**只有 clue giver 的卡片上看得到目標字**，其餘所有人（含主持人分享的畫面）只看得到那四個禁字，所以主持人畫面可以放心分享；目標字在主持人畫面上是藏起來的，需要時可按「👁️ Reveal」（沒有 Firebase 時就靠這顆按鈕，並會顯示提示要其他人先別看）。guesser 的卡片上有「🙋 I got it!」與「🚨 They said one!」兩顆鈕：按下後主持人畫面上那個人的按鈕會亮綠並標「FIRST」（依**主持人端收到的先後**排序），按 foul 的人會列在紅色那顆按鈕上。判定一律由主持人點：點某位 guesser 的名字＝答對，**該 guesser 與 clue giver 各 +1**；點「🚨 X said a forbidden word」或「⏭️ Nobody got it」則這輪 0 分。判定後全部卡片會顯示結果與答案，主持人畫面按鈕變成「▶️ Next Round — ○○ describes」。計分板常駐在下方（輪到的人會highlight），另有「↺ Reset scores」。連結區用 `room.js`，主題色靛 `#6366f1`；頁面最下方列出全部 79 張卡供主持人瀏覽） |
