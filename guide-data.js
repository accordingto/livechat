...
const GAME_GUIDES = {
  ...
  taboo: {
    ...
    sections: [
      ...
      { icon: '🕹️', title: GUIDE_TITLES.how, steps: [
        ...
        { zh: `猜到的人按卡片上的 ${B('is-green', '🙋 Buzz in!')} 然後用講的喊答案。一次只有一個人能搶答：有人搶答時其他人的按鈕會先消失，描述者判定完才回來。`,
          en: `A guesser taps ${B('is-green', '🙋 Buzz in!')} on their card and says the answer out loud. One guess at a time: while it waits on the clue giver, everyone else's buzzer is hidden until the ruling.` },
  ...
  kangaroo: {
    ...
    sections: [
      ...
      { icon: '🕹️', title: GUIDE_TITLES.how, steps: [
        { zh: `任何一位玩家在自己的卡片上按 ${B('is-accent', '⚖️ 抽第一條罪名')} 就開庭（角色隨機發）；你畫面上的 ${B('status-line', '⚖️ 抽一條罪名')} 是備援。陪審團先在卡片上秘密投第一次票，全部投完就自動進入下一步。`,
          en: `Any player taps ${B('is-accent', '⚖️ Draw the first charge')} on their own card to open court (roles are dealt at random); ${B('status-line', '⚖️ Draw a Charge')} on your screen is the backup. The jury casts its secret pre-vote on their cards, and the trial moves on by itself once they all have.` },
        { zh: `檢察官陳述 → 傳喚檢方證人 → 辯護律師答辯 → 傳喚辯方證人（證人從陪審團裡隨機抽，並拿到一張證人卡）。每一步都由正在發言的那一方在自己的卡片上按 ${B('is-accent', '✅ 我講完了')} 推進，不用回頭看你。`,
          en: `Prosecution speaks, calls a witness; defense answers, calls a witness. Witnesses are drawn from the jury and handed a card. Whoever has the floor ends their own step by tapping ${B('is-accent', '✅ Done')} on their card — nobody has to look at you.` },
        { zh: '交互詰問：雙方輪流問被告，被告必須回答每一題。法官（沒有法官時檢辯任一方）在卡片上按「詰問結束」進入最終投票。',
          en: 'Cross-examination: both sides question the defendant, who must answer everything. The Judge (either lawyer when there is no Judge) taps "Close questioning" on their card to open the final vote.' },
        { zh: `陪審團投最終票，全部投完就自動揭曉判決；有法官的局再由法官按一下進入宣判。判決之後任何人按 ${B('is-accent', '⚖️ 下一個案件')}，所有卡片一起跑五秒倒數再開下一案。你的狀態鈕仍然可以推進每一步，按錯可以用 ${B('kc-back-btn', '⬅️ 回上一步')} 退回。`,
          en: `The jury votes again, and the verdict reveals itself once they all have; with a Judge, one tap on the Judge's card moves to the ruling. After that anyone taps ${B('is-accent', '⚖️ Next case')} and every card runs a five-second countdown before the next case. Your status button can still drive any step, and ${B('kc-back-btn', '⬅️ Previous Step')} undoes a misfire.` },
      ]},
      { icon: '📱', title: { zh: '玩家手機上會看到什麼', en: 'What players see on their phones' },
        phones: [
      ...
      { icon: '💡', title: GUIDE_TITLES.tips, tips: [
        ...
        { zh: '全程沒有計時器，節奏由正在發言的人自己掌握：講完按卡片上的「我講完了」就換下一步。',
          en: 'There is no clock. Whoever has the floor sets the pace: they tap "Done" on their card when they are finished.' },
      ]},
    ],
  },
};
