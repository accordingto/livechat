  ...
  taboo: {
    ...
    sections: [
      ...
      { icon: '📱', title: { zh: '每支手機看到的不一樣', en: 'What each phone shows' },
        phones: [
          ...
          { caption: { zh: '🎤 描述者', en: '🎤 Clue giver' },
            card: { name: 'Amy', color: 1,
                    ...
                    buttons: [{ label: { zh: '🛑 我想換一題', en: "🛑 I can't describe this one" }, cls: 'is-quiet' }] } },
        ...
        note: { zh: '整局都在卡片上跑：誰先按都能開下一題，題目要等描述者自己按下去才會出現，倒數也是那時才開始，描述不下去也可以自己換一題。猜題者的卡片上則什麼都沒有——看不到目標字，也看不到禁字。禁字就是最快聯想到答案的字，發給猜的人等於把答案送出去。',
                en: 'The whole round runs from the cards: anyone can deal the next word, the word only appears once the clue giver taps for it (and the clock starts at that moment), and a giver who is stuck can drop it for a new one. The guesser, meanwhile, gets nothing to read — no target word, no forbidden list. Those words are the fastest route to the answer, so handing them over would give the game away.' } },
      { icon: '🎭', title: GUIDE_TITLES.roles, roles: [
      ...
      { icon: '💡', title: GUIDE_TITLES.tips, tips: [
        ...
        { zh: '整局不用碰主持人畫面：發題、開題、搶答、判定、換題都在卡片上，一輪結束也會自己接下一輪。',
          en: 'You never have to touch this screen: dealing, opening, buzzing, ruling and dropping a word all happen on the cards, and a finished round deals the next one itself.' },
      ]},
    ],
  },
